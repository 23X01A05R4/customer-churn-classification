...
The model includes an interactive prediction feature where users can input custom customer data and receive a churn prediction with probability scores.

![Interactive Prediction Terminal](Screenshot.png)

### Model Performance Comparison — ROC Curves

All three models (Logistic Regression, Random Forest, Gradient Boosting) are compared using ROC curves. The Random Forest model achieved the highest AUC score.
...
*   A reusable `predict_churn()` function is provided for making predictions on new customer data.
*   An interactive terminal allows users to input key customer metrics (tenure, monthly charges, contract type) and receive real-time churn predictions with probability scores and risk levels.
...
| **Internet Service** | Fiber optic customers are more likely to churn |
| **Bundled Services** | Online Security and Tech Support reduce churn likelihood |
